...
export const DashboardPreview = () => {
  ...
  const transactions = [
    { label: 'Netflix', sublabel: 'Subscription', amount: '- R$ 49,90', color: 'bg-red-50 text-red-500' },
    { label: 'Pix received', sublabel: 'From Wlad M.', amount: '+ R$ 200,00', color: 'bg-emerald-50 text-emerald-600' },
    { label: 'Electric bill', sublabel: 'Pix payment', amount: '- R$ 127,30', color: 'bg-slate-100 text-slate-500' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-center">
        ...
        >
          ...
          <p className="text-slate-600 text-base md:text-lg mb-8 leading-relaxed">
            Put your savings to work. Earn every day. Pay any bill in seconds — without switching apps.
          </p>
          ...
          >
            ...
            <div className="bg-[#6851FF] px-6 pt-6 pb-10">
              <div className="flex justify-between items-center mb-3">
                <div>
                  <div className="text-white/60 text-xs font-sans mb-0.5">Total balance</div>
              ...
              <div className="inline-flex items-center gap-1.5 bg-white/15 px-3 py-1 rounded-full">
                ...
                <span className="text-emerald-200 text-xs font-sans tabular-nums">Earning 4.2% per year</span>
              </div>
            </div>
            ...
            <div className="px-5 pb-5 -mt-4 bg-white rounded-t-3xl relative z-10">
              ...
              <div className="space-y-2 mt-4">
                <div className="text-[10px] text-slate-400 uppercase tracking-widest font-sans mb-3">Recent payments</div>
                {transactions.map((t, i) => (
                  <div key={i} className="flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-medium text-[#081229]">{t.label}</div>
                        <div className="text-[10px] text-slate-400">{t.sublabel}</div>
                      </div>
                    </div>
                    <span className={`text-sm font-sans tabular-nums font-semibold ${t.amount.startsWith('+') ? 'text-emerald-600' : 'text-[#081229]'}`}>
                      {t.amount}
                    </span>
                  </div>
                ))}
